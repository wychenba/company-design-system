...
CLAUDE.md target ≤ 200(Anthropic best-practice)/ transition ≤ 400 / hard cap 800。SKILL ≤ 250 / spec ≤ 300(foundational SSOT 例外 ≤ 800-1200)/ memory **per-file ≤ 100 lines** + **MEMORY.md index ≤ 20 entries**(soft 18 / hard 20,session-start hook 攔)。Hooks **26 soft / 50 hard**(SSOT = `session_start_governance_check.sh:186`,2026-05-27 升 45→50 per SSOT-P0-not-P1-WARN directive)。動態值見 `scripts/sync-governance-counters.mjs` 跑出為準(2026-05-27:**31 M-rules / 68 audit dims / 48 hooks** — 不寫死避 drift)。
