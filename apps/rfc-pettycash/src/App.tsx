...
const CATEGORIES: Record<string, string[]> = {
  ...
  '訓練/招募/JDP/國內JOS': [
    ...
    'JDP大專院校合作專案', 'JDP-技術研發', '潛在未來人才培育',
    '專案訓練費', '考取證照費', 'JOS子女就學補助', 'JOS搬遷費',
  ],
...
interface LineItem { id: string; category: string; subCategory: string; costCenter: string; accountCode: string; description: string; total: string }
interface InvoiceCard { id: string; number: string; status: StatusKey; payee: string; date: string; voucherType: string; invoiceNo: string; currency: string; subtotal: string; tax: string; total: number; lineItems: LineItem[]; expanded: boolean }
...
interface AddInvoiceModalProps {
  ...
  onComplete: (card: InvoiceCard) => void
}

function AddInvoiceModal({ open, onOpenChange, invoiceNumber, onComplete }: AddInvoiceModalProps) {
  const [step, setStep] = useState<'step1' | 'step2' | 'step3'>('step1')
  const [s1, setS1] = useState({ voucherType: '', date: '', invoiceNo: '', currency: 'TWD', subtotal: '', tax: '' })
  const [s2, setS2] = useState({ category: '', subCategory: '', costCenter: '', accountCode: '', description: '', total: '', taxRate: '5', contractNo: 'none' })
  const [s3, setS3] = useState({ attachType: 'invoice', description: '', fileName: '' })

  const subOpts = s2.category ? (CATEGORIES[s2.category] ?? []).map((v) => ({ value: v, label: v })) : []
  const stepOrder = ['step1', 'step2', 'step3'] as const
  const idx = stepOrder.indexOf(step)

  function reset() {
    setStep('step1')
    setS1({ voucherType: '', date: '', invoiceNo: '', currency: 'TWD', subtotal: '', tax: '' })
    setS2({ category: '', subCategory: '', costCenter: '', accountCode: '', description: '', total: '', taxRate: '5', contractNo: 'none' })
  ...
  function handleSubmit() {
    onComplete({
      id: Date.now().toString(), number: invoiceNumber, status: 'draft',
      payee: '林間宜 (023156)', date: s1.date || '2026/05/25',
      voucherType: VOUCHER_TYPES.find((v) => v.value === s1.voucherType)?.label ?? '',
      invoiceNo: s1.invoiceNo, currency: s1.currency,
      subtotal: s1.subtotal, tax: s1.tax,
      total: parseFloat(s1.subtotal || '0') + parseFloat(s1.tax || '0'),
      lineItems: s2.category ? [{ id: '1', category: s2.category, subCategory: s2.subCategory, costCenter: s2.costCenter, accountCode: s2.accountCode, description: s2.description, total: s2.total }] : [],
      expanded: true,
    })
    handleClose()
  }
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field>
                  ...
                  <Input placeholder="填寫日期" value={s1.date} onChange={(e) => setS1((p) => ({ ...p, date: e.target.value }))} />
                </Field>
                <Field>
                  <FieldLabel>發票號碼</FieldLabel>
                  <Input placeholder="填寫發票號碼" value={s1.invoiceNo} onChange={(e) => setS1((p) => ({ ...p, invoiceNo: e.target.value }))} />
                </Field>
              </div>
              <Field>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field>
                  ...
                  <Input value={s1.subtotal} onChange={(e) => setS1((p) => ({ ...p, subtotal: e.target.value }))} />
                </Field>
                <Field>
                  <FieldLabel>稅額 <InfoTooltip content="稅額 = 合計金額 × 稅率" /></FieldLabel>
                  <Input value={s1.tax} onChange={(e) => setS1((p) => ({ ...p, tax: e.target.value }))} />
                </Field>
              </div>
              <div className="grid grid-cols-2 gap-4 rounded-lg bg-surface-raised p-3 text-body">
                <div>
                  <div className="text-caption text-fg-secondary">稅後金額</div>
                  <div>{s1.subtotal ? String(parseFloat(s1.subtotal) + parseFloat(s1.tax || '0')) : '-'}</div>
          ...
          {step === 'step2' && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4 rounded-lg bg-surface-raised p-3 text-body">
                <div><div className="text-caption text-fg-secondary mb-1">請款單號</div><div className="font-medium">{invoiceNumber}</div></div>
                <div><div className="text-caption text-fg-secondary mb-1">發票號碼</div><div>{s1.invoiceNo || '-'}</div></div>
                <div><div className="text-caption text-fg-secondary mb-1">序號</div><div>1</div></div>
              </div>
              <Notice variant="info">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field>
                  ...
                  <Select placeholder="請選擇" options={subOpts} value={s2.subCategory} onChange={(v) => setS2((p) => ({ ...p, subCategory: v as string }))} disabled={!s2.category} />
                </Field>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Field>
              ...
              <Field><FieldLabel>描述</FieldLabel><Input value={s2.description} onChange={(e) => setS2((p) => ({ ...p, description: e.target.value }))} /></Field>
              <div className="grid grid-cols-3 gap-4">
                <Field>
                  <FieldLabel required>總額</FieldLabel>
                  <Input placeholder="填寫總額" value={s2.total} onChange={(e) => setS2((p) => ({ ...p, total: e.target.value }))} />
                </Field>
                <Field>
                ...
                <Field>
                  ...
                  <Input readOnly value={s2.total && s2.taxRate === '5' ? String(Math.round(parseFloat(s2.total) * 0.05)) : ''} />
                </Field>
              </div>
              <Field>
                <FieldLabel>是否提供合約編號</FieldLabel>
                <RadioGroup value={s2.contractNo} onValueChange={(v) => setS2((p) => ({ ...p, contractNo: v }))} orientation="horizontal">
          ...
          <div className="flex w-full items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              {idx < 2
                ...
                : <Button variant="primary" size="md" onClick={handleSubmit}>新增</Button>
              }
            </div>
          </div>
        </DialogFooter>
                ...
                <div className="border-t border-divider px-4 py-3 space-y-2">
                  {invoices.map((inv) => (
                    <div key={inv.id} className="rounded-lg border border-divider p-3">
                      ...
                      <div className="text-caption text-fg-secondary mt-1">{inv.voucherType} | {inv.date}</div>
                    </div>
                  ))}
              ...
              {expanded.attachments && (
                <div className="border-t border-divider px-4 py-3">
                  {attachments.length === 0 ? (
                    <span className="text-body text-fg-secondary">尚未上傳附件</span>
                  ) : (
                    ...
                    <tbody className="divide-y divide-divider">
                      {[
                        ...
                        <tr key={row.role}>
                          ...
                          <td className="py-2 text-fg-tertiary">-</td>
                          <td className="py-2 text-fg-tertiary">-</td>
                          <td className="py-2 text-fg-tertiary">-</td>
                          <td className="py-2 text-fg-tertiary">-</td>
...
// ─────────────────────────────────────────────────────────────
// Invoice Card Row (within form)
// ─────────────────────────────────────────────────────────────

function InvoiceCardRow({ card, onToggle, onDelete }: { card: InvoiceCard; onToggle: () => void; onDelete: () => void }) {
      ...
      {card.expanded && (
        <div className="border-t border-divider px-4 py-3 space-y-3">
          <div className="grid grid-cols-4 gap-4 text-body">
            <div><div className="text-caption text-fg-secondary">憑證類型</div><div className="truncate">{card.voucherType || '-'}</div></div>
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="secondary" size="sm" startIcon={Plus}>新增細項</Button>
                </div>
              </div>
              <table className="w-full text-body">
                <thead>
                    ...
                    <tr key={li.id}>
                      ...
                      <td className="py-2">
                        <div className="flex gap-1">
                          <Button variant="ghost" size="sm" startIcon={Pencil} iconOnly aria-label="編輯" />
                          <Button variant="ghost" size="sm" startIcon={Trash2} iconOnly aria-label="刪除" />
                        </div>
                      </td>
                    </tr>
                  ))}
...
function CreateFormPage({ formId, onBack }: { formId: string; onBack: () => void }) {
  ...
  const nextNumber = `${formId}-${invoices.length + 1}`

  function handleAddInvoice(card: InvoiceCard) {
    setInvoices((prev) => [...prev, { ...card, number: nextNumber }])
  }
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-auto bg-surface-subtle">
          <div className="max-w-4xl mx-auto px-6 py-6 space-y-4">
            ...
            <div className="rounded-xl border border-divider bg-surface p-6 space-y-4">
              ...
              <Button variant="secondary" size="md" startIcon={Plus} onClick={() => setShowAddInvoice(true)}>新增請款</Button>
              {invoices.length === 0 ? (
      ...
      <AddInvoiceModal open={showAddInvoice} onOpenChange={setShowAddInvoice} invoiceNumber={nextNumber} onComplete={handleAddInvoice} />
      <AddAttachmentModal open={showAddAttachment} onOpenChange={setShowAddAttachment} onComplete={(a) => setAttachments((prev) => [...prev, a])} />
      <PreviewModal open={showPreview} onOpenChange={setShowPreview} invoices={invoices} attachments={attachments} onSubmit={handleSubmit} />
    </>
  )
}
