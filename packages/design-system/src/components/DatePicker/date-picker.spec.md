...
- 用 `mode="single"` + 自管 `rangeModifiers`(**不**用 RDP 內建 `mode="range"`——其 click 配對邏輯與 activeEnd 衝突,見 date-picker.tsx L979-981);`numberOfMonths={showTime ? 1 : 2}`(date-only Range 兩月並列對齊 [Ant RangePicker](https://ant.design/components/date-picker#rangepicker);showTime Range 1 月,對齊 Ant `multiplePanel=false` <!-- @benchmark-unverified: Airbnb / Booking 無公開 DS spec URL,visual observation -->)
...
- range track 視覺由已選端點靜態算出(`rangeModifiers` 讀 committed start/end,見 date-picker.tsx L781-793);**無** hover 未選端點的預覽(RDP hover-preview 只在內建 `mode="range"` 才有,本元件刻意不用)
