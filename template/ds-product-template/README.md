...
**為何要密碼?** Storybook 含內部 product UI,Netlify deploy 預設公開,要擋陌生人。**免費做法 = HTTP Basic Auth via `_headers`**(所有 Netlify 方案含 free-tier 都支援,edge 層擋,瀏覽器原生帳密彈窗)。本 template 已內建:`netlify.toml` build 後跑 `scripts/inject-basic-auth.mjs`,從 Netlify env var `STORYBOOK_BASIC_AUTH`(格式 `user:password`,多組空格分隔)寫 `storybook-static/_headers`;未設 = 站台公開(no-op)。

> Netlify Dashboard 的「Password protection / Basic protection」(Site settings → Access & security)是 **Pro 方案專屬**($20/mo),free-tier 沒這個開關,按下去會被要求升級付費——所以免費路徑走上面的 env var,不走 dashboard。
...
**Step 2 — 🔒 設 STORYBOOK_BASIC_AUTH 上密碼**(免費,所有 Netlify 方案支援):

免費的 access control = **HTTP Basic Auth via `_headers`**(edge 層擋,瀏覽器原生帳密彈窗)。本 template 已內建:`netlify.toml` build 後跑 `scripts/inject-basic-auth.mjs`,從 Netlify env var `STORYBOOK_BASIC_AUTH` 寫 `storybook-static/_headers`。fork user 設定 30 秒:
...
> 密碼只存 Netlify 後台 env var + edge,**不進 public repo**(public repo 不能 commit 明文帳密);未設 env var = 站台公開(no-op)。

**為何不走 Dashboard 的 Password Protection?**(2026-06-05 官方 docs 證實):
...
- ✅ **`STORYBOOK_BASIC_AUTH` env var → `_headers` Basic Auth** = 免費、所有方案支援,本 template 已內建

**Defense-in-depth**(`netlify.toml` 已 ship):X-Robots-Tag noindex(搜尋引擎不收錄 URL)+ Referrer strict-origin + X-Frame SAMEORIGIN — 只防 SEO 索引,不擋直接訪問;**真實擋人**靠上面的 `STORYBOOK_BASIC_AUTH` Basic Auth 那一層。
