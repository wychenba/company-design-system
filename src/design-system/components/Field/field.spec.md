...
`naked` = cell-as-input substrate(Notion / Airtable / Excel / AG Grid 共識:host cell 提供 box,Field 融入)。

**4 層 SSOT(`field-wrapper.tsx` const export,所有 cell-as-input 元件必 import)**:
1. **`nakedCellRowModeAlign`** — host cell `data-row-mode` 自動 propagate alignment(autoRow→items-start / fixed→items-center)
2. **`nakedCellSuffixSlot`** — chevron / endIcon wrapper:autoRow 自動 `self-start + h-[1lh] + flex items-center`,中心對齊 value 第 1 行(對齊 Combobox wrap mode canonical:Notion/Airtable 共識)
3. **`nakedCellHoverRing` / `nakedCellFocusRing` / `nakedCellOpenRing`** — state ring 用 `outline-2 outline-offset-[-1px] outline-{border|primary}` straddle cell edge(完整蓋 4 邊 grid divider 不留漏邊;不用 `border`(吃高度)/ 不用 `box-shadow inset`(只內側,不蓋 adjacent grid))
4. **token-level SSOT**:hover/open 走 `--border`(同 Field bare),focus 走 `--primary`(同 Field default + bare)— **改 token 值 → Field 三變體全跟動**

**反模式 ❌**:hardcode `items-center` / `border` ring / `box-shadow inset` ring / 自寫 outline class(必走 SSOT const)。

**3 hook 攔截**:`check_naked_row_mode_propagation.sh` / `check_field_state_token_consume.sh` / `check_inline_action_canonical_gap.sh`(write-time block)+ `/design-system-audit` Group N M36+M37+M38(periodic batch verify)。
