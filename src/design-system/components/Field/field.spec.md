...
`naked` = cell-as-input substrate(Notion / Airtable / Excel / AG Grid:host cell 提供 box,Field 融入)。**3 層 SSOT 分工**(2026-05-05 v8 — slot 幾何下沉到 L1 primitive):

| Layer | Home | 內容 |
|---|---|---|
| **L1 Slot 幾何** | `patterns/element-anatomy/item-anatomy.tsx` | `<ItemPrefix>` / `<ItemSuffix>`(永遠 `h-[1lh]` 對齊第 1 行,單行視覺 = items-center)。startIcon / value-icon 用 ItemPrefix;chevron / clear / calendar / endAction 用 ItemSuffix |
| **L2 Wrapper align** | `field-wrapper.tsx` `nakedCellRowModeAlign` | host cell `data-row-mode` propagate(autoRow→items-start / fixed→items-center 預設)|
| **L3 State ring** | `field-wrapper.tsx` `nakedCellHoverRing/FocusRing/OpenRing` | `outline-2 outline-offset-[-1px] outline-{border\|primary}` straddle cell edge,token-level SSOT(`--border`/`--primary` 改值三變體全動)。Field-naked 專屬不下沉(MenuItem/TreeView 走 bg hover)|

**前身已 retire**(2026-05-05 v8):`nakedCellPrefixSlot` / `nakedCellSuffixSlot` 是平行 SSOT(M1+M17 違反),改消費 L1。

**反模式 ❌**:hardcode `<span class="h-[1lh]...">` slot(用 L1)/ hardcode `items-center` wrapper(用 L2)/ `border` 或 `box-shadow inset` ring(用 L3)。

**機械防漂移**:hook `check_naked_row_mode_propagation` / `check_field_state_token_consume` / `check_inline_action_canonical_gap` / `check_row_slot_handcraft`(write-time)+ audit `/design-system-audit` Group N M36-M39(periodic)。
