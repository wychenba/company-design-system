...
**Layout Family**：非 4-Family Model —— **self-contained primitive**（獨立視覺，無 slot 結構，類似 Switch / Checkbox / Badge / Spinner）。
...
| 進度顯示（任務完成度、上傳進度）| Progress bar（未來）| Rating 不是進度指標，別誤用星星做「完成 4/5 步」|
...
| Size | Icon 尺寸 | 使用情境 |
|------|-----------|---------|
| `sm` | 16px | 商品列表、搜尋結果、DataTable cell 內的星等展示（空間緊）|
| `md` | 20px | **預設**。一般卡片、評論列表、商品詳情頁 |
| `lg` | 24px | 送出評分的 review form、強調的主 CTA 區塊 |

### 為什麼不走 field-height family

Rating 不是 field control（不是單行 editable data input），是 **pure visual indicator**——高度由 icon 尺寸自然撐起，沒有 padding / border box 需要對齊 Field。因此 size 對應的是 **icon px**（16/20/24，跟 Lucide icon 標準尺寸同步），不是 `--field-height-*`。

若 Rating 要放在 Field 內（例如 `<Field label="評分"><Rating /></Field>`），Field 的高度由 Rating 內容撐開，不需要 Rating 自己配 field-height。
