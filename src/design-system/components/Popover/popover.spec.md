...
**Content size canonical（2026-04-29）**:density lock md → **body content fields（Input / Select / Checkbox / row 內容）預設 `size="md"`**（field-height-md 32px,對齊 density）。**Chrome buttons（close X / refresh / footer action button）維持 `size="sm"`**（輕量 chrome canonical）。Row icon action（同 row inline action）用 `ItemInlineActionButton size="md"`（16+18 hover bg,對齊 inline-action.spec.md 尺寸對照「Panel list row」）。
