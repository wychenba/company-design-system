...
**Close X 按鈕(2026-04-22 v4:unbounded canonical)**:所有 `PopoverHeader` 內建右上 X 按鈕,**size="xs"**(dismiss 永遠 unbounded,xs 固定 24 匹配 chrome-header-height 幾何 — 詳 `patterns/overlay-surface/overlay-surface.spec.md`「Chrome dismiss size canonical」)。實作 = `<Button iconOnly dismiss size="xs" startIcon={X} aria-label="關閉" />`(非 Inline Action / 非自刻 button)。`hideClose` prop 可讓 composition 元件(如 Coachmark)選擇隱藏。

**Header / Footer 高度 canonical**:透過 `SurfaceHeader` / `SurfaceFooter` primitive 提供的 padding-based sizing(`py-[var(--layout-space-tight)]`),高度 = max(child) + 2×tight。**Unbounded button xs(24)+ tight × 2 = 48 md / 56 lg = `--chrome-header-height` ✓**;若 footer 塞 bounded button(primary / tertiary sm),高度自然長(28+ × 12/16 × 2),因 bounded 有視覺邊界 padding 顯得自然。跟 Dialog / Sheet / Coachmark 完全一致 — **overlay family 統一 padding-based + unbounded/bounded 共通 canonical**。
