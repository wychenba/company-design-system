...
**SurfaceHeader 刻意設計為 padding-based**(`py-tight`,**無** 全域 min-h)。Dialog/Sheet title `text-body-lg` line-height 24 = unbounded button slot 24,padding-based 自然穩定 = 48 chrome-header-height md。**Popover 等 non-modal 輕量浮層走 `LIGHTWEIGHT_CHROME_HEADER`**(`min-h-10` 40 + `!py-2` 16 → inner 24 匹配 button slot,title 14 line-height 20 在 24 內垂直置中);世界級對照 Linear / Notion / Figma popover header 共識(2026-05-04 Q10 重思)。SurfaceFooter 對稱。**語義宣告**:
