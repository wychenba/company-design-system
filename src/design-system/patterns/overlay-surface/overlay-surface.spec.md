...
**SurfaceHeader 刻意設計為 padding-based**(`py-tight` + 顯式 `min-h-[calc(var(--field-height-xs)+var(--layout-space-tight)*2)]` 鎖 chrome-header-height = 48 md / 56 lg)。**Min-h 補丁(2026-05-04 Q10)**:過去靠 unbounded slot trick 間接達成,但「title-only / refresh 動態消失」case title (line-height 20) 不撐 24,header 塌 44px → 與「title + close」48px 衝突,違 M5 state 疊加 canonical。SurfaceFooter 對稱同治。**語義宣告**:
