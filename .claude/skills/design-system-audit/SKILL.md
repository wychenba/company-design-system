...
| 37 | **Naked variant state ring token SSOT**(M19,2026-05-05 v4 新增)| DS-wide grep cell-as-input naked variant 元件,確認 state ring **走 `nakedCellHoverRing` / `nakedCellFocusRing` / `nakedCellOpenRing` SSOT const**(`field-wrapper.tsx` export),不可自寫 `box-shadow inset` / 不可 hardcode outline token。token-level SSOT:hover/open=`--border`(同 Field bare),focus=`--primary`。改 token → Field default + bare + naked 三變體全跟動。Hook `check_field_state_token_consume.sh` write-time 攔。違反 = state ring 漂移、grid divider 無漏邊蓋失效 |
